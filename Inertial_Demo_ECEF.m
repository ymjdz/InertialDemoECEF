...
% Improved 5/5/16 by Jiang Dongzhi
...
input_profile_name='Test2_60sec_data.pos';
%<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<%
...
initialization_errors.delta_eul_nb_n = [0;0;0]*deg_to_rad;
...
[in_profile,no_epochs,ok] = Read_pos_profile(input_profile_name);
%<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<%
...
[out_profile,out_errors,in_profile] = Inertial_navigation_ECEF(in_profile,no_epochs,...
...
Plot_comparison(in_profile,out_profile);
