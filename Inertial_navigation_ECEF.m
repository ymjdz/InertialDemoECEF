function [out_profile,out_errors,in_profile] = Inertial_navigation_ECEF(in_profile,...
...
for epoch = 2:no_epochs
    ...
    %对误差做超限判断
    if abs(delta_r_eb_n(1))>=5 || abs(delta_r_eb_n(2))>=5 || abs(delta_r_eb_n(3))>=5
        break;
    end
...
%删除误差超限历元之后的所有历元
out_profile = out_profile(1:epoch,:);
out_errors = out_errors(1:epoch,:);
in_profile = in_profile(1:epoch,:);
